...
import { AccountWithdrawByNumber } from 'src/app/models/account/withdraw/account-withdraw-by-number';
...
export class WithdrawComponent {
  ...
  submit(){
    ...
    if(isAmountValid){
      StaticData.accountWithdraw = new AccountWithdrawByNumber(StaticData.account.accountNumber, amount);
    ...
    let amount = parseInt(StaticData.accountWithdraw.amount);
      this.accountService.getAccountByAccountNumber(StaticData.account).subscribe(response=>{
        ...
        if(response.balance > amount){
          this.accountService.withdrawByNumber(StaticData.accountWithdraw).subscribe(withdrawRes=>{
