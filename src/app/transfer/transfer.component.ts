...
export class TransferComponent {
  ...
  performFundTransfer(recAc:string, amount:string){
    this.accountService.getAccountByAccountNumber(new AccountNumber(recAc)).subscribe(response=>{
      let account = response;
      let proceed = confirm("Transfer Rs. "+ amount + "to " + account.accountHolderName);
      if(!proceed){
        return;
      }
      let myAccount = new AccountNumber(StaticData.account.accountNumber);
      this.accountService.getAccountByAccountNumber(myAccount).subscribe(myAcResponse=>{
        ...
        this.accountService.fundTransfer(new FundTransferForm(StaticData.account.accountNumber, recAc, amount)).subscribe(fundTransferResponse=>{
